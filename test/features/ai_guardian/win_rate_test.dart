...
void main() {
  test('no history yields no rate', () {
    final r = AIGuardianService.winRateFor([]);
  ...
  test('too few trades yields no rate rather than a noisy one', () {
    ...
    final r = AIGuardianService.winRateFor(trades([10, 20, 30, 40]));
    expect(r.rate, isNull);
    expect(r.sample, 4);
  });

  test('computes the rate once there is a usable sample', () {
    final r = AIGuardianService.winRateFor(trades([10, -5, 20, -8, 30]));
    expect(r.rate, closeTo(60, 1e-9));
    expect(r.sample, 5);
  });

  test('all losses reports 0, not null', () {
    final r = AIGuardianService.winRateFor(trades([-1, -2, -3, -4, -5]));
    expect(r.rate, 0);
    expect(r.sample, 5);
  });
  ...
  test('breakeven trades count as losses, not wins', () {
    ...
    final r = AIGuardianService.winRateFor(trades([0, 0, 0, 0, 10]));
